...
import LoadingProcessor from "./LoadingProcessor";
...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
  const { role, content, type, action, created_at, intent, grounded, guardrail_flags, isProcessing } = message;
  ...
  return (
    <div
      className={cn("flex flex-col space-y-1 max-w-[88%] select-none text-left", {
    ...
    >
      ...
      {/* Main Message Bubble or Loading Processor */}
      {!isUser && (isProcessing || !content) ? (
        ...
        >
          ...
          {/* Response Metadata Badges (Intent, Grounded, Guardrail Flags) */}
          {!isUser && (intent || grounded !== undefined || (guardrail_flags && guardrail_flags.length > 0)) && (
            <div className="flex flex-wrap items-center gap-1.5 mt-3 pt-2.5 border-t-[0.5px] border-sand/30">
              {intent && (
                <span className="inline-flex items-center gap-1 text-[10px] font-mono font-semibold text-sage-deep bg-sage/30 px-2 py-0.5 rounded border border-sage-deep/15">
                  <Zap size={10} className="text-sage-deep" />
                  <span>Intent: {intent}</span>
                </span>
              )}
              {grounded === true && (
                <span className="inline-flex items-center gap-1 text-[10px] font-mono font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200/80">
                  <ShieldCheck size={10} className="text-emerald-600" />
                  <span>Fact Grounded</span>
                </span>
              )}
              {guardrail_flags && guardrail_flags.length > 0 && (
                <span className="inline-flex items-center gap-1 text-[9.5px] font-mono text-ink-ghost bg-sand/30 px-1.5 py-0.5 rounded border border-sand/40" title={`Guardrail flags: ${guardrail_flags.join(", ")}`}>
                  <span>🔒 Guardrails ({guardrail_flags.length})</span>
                </span>
              )}
            </div>
          )}
          ...
          {!isUser && content && (
            <div className="space-y-3 mt-3 border-t-[0.5px] border-sand/20 pt-2.5">
              {/* ACTION CARD */}
